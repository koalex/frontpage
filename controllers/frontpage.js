...
function onAssetsChange (filePath) {
    if (!filePath) {
        try {
            assets = JSON.parse(fs.readFileSync(path.join(config.staticRoot, 'assets.json'),   'utf8'));
            favicons = JSON.parse(fs.readFileSync(path.join(config.staticRoot, 'favicons.json'), 'utf8'));
